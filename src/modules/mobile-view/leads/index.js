...
class Category extends BaseComponent {
  ...
  getAllLeads = async () => {
    let request = {
      ...
      district: this.props.selectedDistrict.toLowerCase(),
  ...
  backToSelectCategory = async () => {
    ...
    // history.push(`/${this.props.selectedState}/${this.props.selectedDistrict}`);
  };
  ...
  sendUpVoteRequest = async (id, desription) => {
    ...
    try {
      ...
      if (
        ...
        this.setState({
          allLeads: response.responseData,
  ...
  incrementUpVote = (id, comments = []) => {
    ...
    this.state.allLeads[index].upVoteCount =
      this.state.allLeads[index].upVoteCount + 1;
    this.state.allLeads[index].comments = comments;
    this.setState({
      allLeads: this.state.allLeads,
      originalResponseData: this.state.allLeads,
    });
  };

  incrementDownVote = (id, comments = []) => {
    let index = this.state.responseByIndex[id].index;
    this.state.allLeads[index].downVoteCount =
      this.state.allLeads[index].downVoteCount + 1;
    this.state.allLeads[index].comments = comments;
    this.setState({
      allLeads: this.state.allLeads,
      originalResponseData: this.state.allLeads,
