...
const LeadDetailsComponent = (props) => {
  ...
  const { isVolunteerViewType } = props;
  const { leadDetails } = props.state;
  const url =
    leadDetails?.category && leadDetails?.state
      ? `https://app.missionhumane.org/${
          leadDetails?.state
        }/${leadDetails?.category?.toLowerCase()}`
      : leadDetails?.state
      ? `https://app.missionhumane.org/${leadDetails?.state}`
      : `https://app.missionhumane.org/`;
  const leadDesc = leadDetails?.description
    ? linkify(leadDetails?.description)
    : "";
      ...
      {Object.keys(leadDetails).length ? (
        <div>
          <div
            ...
            style={{ maxWidth: "90%", margin: "auto", paddingBottom: "14vh" }}
          >
            {isVolunteerView && (
              <>
                  ...
                  <Column>
                    <b>
                      {leadDetails?.type === ""
                        ? "YOU CAN HELP"
                        : "YOU CAN HELP"}{" "}
                      -pending({props.state.count})
                    </b>
                  </Column>
                  <Column>
                    <button
                      className="bt-refresh"
                      onClick={props.getVolunteerDetails}
                    >
                      Next
                    </button>
                  </Column>
                ...
                <div className="p-b-10">
                  {leadDetails?.type === "" ? (
                    <p className="p-t-20 fc-custom-blue fs-14">
                      Please call the number and find out if this information is
                      working or not working; after finishing the call, click on
                      the Working or Not Working.
                    </p>
                  ) : (
                    <p className="p-t-20 fc-custom-blue fs-14">
                      Please go through the list of resources on
                      <a
                        target="_blank"
                      ...
                      and find a relevant source for this following request. If
                      you have found some help, please reach out to them and
              ...
              <>
                <Row className="justify-content-between">
                  ...
                  <Column>
                    <button
                      className="bt-refresh"
                      onClick={() => props.getVolunteerDetailsType(type)}
                    >
                      Next
                    </button>
                  </Column>
            ...
            typeof props.state?.leadDetails.attachments.media_keys ===
              "object" &&
            props.state?.leadDetails.attachments.media_keys.length ? (
              <>
                {props.state?.uniqueContact &&
                props.state?.uniqueContact.length > 0 ? (
                ...
                <Row className="card-timestamp">
                  <Column>
                    <div className="p-t-20 p-r-20 p-b-10 bottom-text">
                      {(props.state?.leadDetails?.comments?.length > 0 &&
                        props.state?.leadDetails.comments.map(
                          (comment, index) =>
                            comment.type === voteTypeConstants.DOWN_VOTE && (
                              <Typography
                                ...
                                variant="body2"
                              >
                                <span className="p-t-20 bottom-text">
                                  {moment(comment.addedOn).fromNow()}
                                </span>{" "}
                        ...
                        moment(
                          props.state?.leadDetails?.channelCreatedOn || ""
                        ).fromNow()}
                    </div>
                  </Column>
                  <Column>
                    <div className="p-t-20 p-r-20 p-b-10 bottom-text">
                      {leadDetails?.district
                        ? Array.isArray(leadDetails?.district)
              ...
              <div className="selected-item ">
                <div className="p-l-20 p-t-20 p-r-20 fs-14">{leadDesc}</div>
                {props.state?.uniqueContact &&
                props.state?.uniqueContact.length > 0 ? (
                ...
                <Row className="card-timestamp">
                  <Row
                    className="p-l-20 p-t-20 p-r-20 p-b-10 bottom-text w-100"
                  ...
                  {props.state?.leadDetails?.comments?.length < 1 && (
                    <Column>
                      <div className="p-l-20 p-t-20 p-r-20 p-b-10 bottom-text">
                        {`Source: ${utility.toSentenceCase(
                          props.state?.leadDetails?.channel || ""
                        )}`}
                      </div>
                    </Column>
                  )}
          ...
          {props.state.isShowNotWorkingPopup && (
            <NotWorkingDialog
              state={props.state}
          ...
          {props.state.isShowWorkingPopup && (
            <WorkingDialog
              state={props.state}
